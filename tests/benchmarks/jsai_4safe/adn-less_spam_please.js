...
function three(event) { Components.classes['@mozilla.org/preferences-service;1'].getService(Components.interfaces.nsIPrefBranch).getBranch().setIntPref('extensions.lessspamplease.addresstype', this.value)
}
document.addEventListener("command", three, false);

function four(event) { this.value=Components.classes['@mozilla.org/preferences-service;1'].getService(Components.interfaces.nsIPrefBranch).getBranch().getIntPref('extensions.lessspamplease.addresstype')
}
document.addEventListener("command", four, false);

function five(event) { Components.classes['@mozilla.org/preferences-service;1'].getService(Components.interfaces.nsIPrefBranch).getBranch().setCharPref('extensions.lessspamplease.mailprovider', this.value);
}
document.addEventListener("command", five, false);

function six(event) { this.value=Components.classes['@mozilla.org/preferences-service;1'].getService(Components.interfaces.nsIPrefBranch).getBranch().getCharPref('extensions.lessspamplease.mailprovider');
}
