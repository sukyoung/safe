...
(function () {
    ...
    QUnit.test('should work with a `start` and `end`', function (assert) {
        ...
        assert.strictEqual(_.inRange(__num_top__, 1, 5), true);
        assert.strictEqual(_.inRange(__num_top__, 1, 5), false);
        assert.strictEqual(_.inRange(5, 1, __num_top__), false);
    });
    QUnit.test('should treat falsey `start` as `0`', function (assert) {
        ...
        lodashStable.each(falsey, function (value, index) {
            if (index) {
                ...
                assert.strictEqual(_.inRange(__num_top__, value, 1), __bool_top__);
            } else {
                assert.strictEqual(_.inRange(__num_top__), false);
            }
        });
    });
    ...
    QUnit.test('should work with a floating point `n` value', function (assert) {
        assert.expect(4);
        assert.strictEqual(_.inRange(__num_top__, __num_top__), true);
        assert.strictEqual(_.inRange(1.2, __num_top__, 5), true);
