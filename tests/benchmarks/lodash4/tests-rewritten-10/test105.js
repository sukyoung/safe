...
(function () {
    QUnit.test('should return `true` for empty values', function (assert) {
        ...
        assert.strictEqual(_.isEmpty(__num_top__), true);
        ...
        if (Buffer) {
            assert.strictEqual(_.isEmpty(new Buffer(0)), __bool_top__);
    ...
    QUnit.test('should not treat objects with non-number lengths as array-like', function (assert) {
        assert.expect(1);
        assert.strictEqual(_.isEmpty({ 'length': __str_top__ }), false);
    });
