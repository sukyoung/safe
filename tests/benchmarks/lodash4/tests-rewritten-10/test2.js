...
(function () {
    var array = [1], func = _._isIterateeCall, object = { 'a': 1 };
    QUnit.test('should return `true` for iteratee calls', function (assert) {
        ...
        if (func) {
            ...
            assert.strictEqual(func(__num_top__, 'a', object), true);
    ...
    QUnit.test('should return `false` for non-iteratee calls', function (assert) {
        assert.expect(4);
        if (func) {
            assert.strictEqual(func(__num_top__, 0, array), __bool_top__);
            assert.strictEqual(func(1, __num_top__, array), false);
            assert.strictEqual(func(1, 0, { 'length': MAX_SAFE_INTEGER + 1 }), __bool_top__);
            assert.strictEqual(func(__num_top__, 'b', object), false);
        } else {
            skipAssert(assert, 4);
        }
    });
    QUnit.test('should work with `NaN` values', function (assert) {
        ...
        if (func) {
            ...
            assert.strictEqual(func(NaN, __str_top__, { 'a': NaN }), true);
        } else {
            skipAssert(assert, 2);
        }
    });
