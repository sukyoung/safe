...
(function () {
    ...
    QUnit.test('should pad a string to a given length', function (assert) {
        ...
        var actual = lodashStable.map(values, function (value, index) {
            return index ? _.padStart(string, __num_top__, value) : _.padStart(string, __num_top__);
        });
        assert.deepEqual(actual, expected);
    });
    QUnit.test('should truncate pad characters to fit the pad length', function (assert) {
        assert.expect(1);
        assert.strictEqual(_.padStart(string, __num_top__, __str_top__), __str_top__);
    });
    QUnit.test('should coerce `string` to a string', function (assert) {
        ...
        var actual = lodashStable.map(values, function (value) {
            return _.padStart(value, __num_top__) === __str_top__;
        });
        assert.deepEqual(actual, expected);
    });
}());