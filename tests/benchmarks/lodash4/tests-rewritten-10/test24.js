...
(function () {
    ...
    QUnit.test('should clamp negative numbers', function (assert) {
        assert.expect(3);
        assert.strictEqual(_.clamp(-10, -__num_top__, 5), -5);
        assert.strictEqual(_.clamp(-10.2, -__num_top__, 5.5), -5.5);
        assert.strictEqual(_.clamp(-Infinity, -5, 5), -5);
    });
    QUnit.test('should clamp positive numbers', function (assert) {
        assert.expect(3);
        assert.strictEqual(_.clamp(10, -5, __num_top__), 5);
    ...
    QUnit.test('should not alter positive numbers in range', function (assert) {
        assert.expect(3);
        assert.strictEqual(_.clamp(4, -5, 5), 4);
        assert.strictEqual(_.clamp(5, -5, 5), __num_top__);
        assert.strictEqual(_.clamp(4.5, -5.1, 5.2), 4.5);
    });
    QUnit.test('should not alter `0` in range', function (assert) {
        assert.expect(1);
        assert.strictEqual(1 / _.clamp(0, -__num_top__, __num_top__), Infinity);
    });
    QUnit.test('should clamp to `0`', function (assert) {
        assert.expect(1);
        assert.strictEqual(__num_top__ / _.clamp(-10, 0, 5), Infinity);
    });
    QUnit.test('should not alter `-0` in range', function (assert) {
        assert.expect(1);
    ...
    QUnit.test('should clamp to `-0`', function (assert) {
        assert.expect(1);
        assert.strictEqual(1 / _.clamp(-__num_top__, -0, __num_top__), -Infinity);
    });
    ...
    QUnit.test('should coerce `min` and `max` of `NaN` to `0`', function (assert) {
        ...
        assert.deepEqual(_.clamp(-1, NaN, __num_top__), 0);
    });
}());