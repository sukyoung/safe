...
(function () {
    ...
    QUnit.test('should clamp negative numbers', function (assert) {
        ...
        assert.strictEqual(_.clamp(-Infinity, -__num_top__, 5), -5);
    });
    ...
    QUnit.test('should not alter negative numbers in range', function (assert) {
        assert.expect(3);
        assert.strictEqual(_.clamp(-4, -5, 5), -__num_top__);
        assert.strictEqual(_.clamp(-5, -__num_top__, 5), -5);
        assert.strictEqual(_.clamp(-5.5, -5.6, 5.6), -__num_top__);
    });
    QUnit.test('should not alter positive numbers in range', function (assert) {
        assert.expect(3);
        assert.strictEqual(_.clamp(__num_top__, -5, 5), __num_top__);
        assert.strictEqual(_.clamp(5, -5, __num_top__), 5);
        assert.strictEqual(_.clamp(__num_top__, -5.1, 5.2), 4.5);
    });
    QUnit.test('should not alter `0` in range', function (assert) {
        assert.expect(1);
        assert.strictEqual(__num_top__ / _.clamp(0, -5, 5), Infinity);
    });
    ...
    QUnit.test('should return `NaN` when `number` is `NaN`', function (assert) {
        assert.expect(1);
        assert.deepEqual(_.clamp(NaN, -5, __num_top__), NaN);
    });
