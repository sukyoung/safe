...
(function () {
    ...
    QUnit.test('should mixin `source` methods into lodash', function (assert) {
        ...
        if (!isNpm) {
            _.mixin(source);
            assert.strictEqual(_.a(array), __str_top__);
    ...
    QUnit.test('should use a default `object` of `this`', function (assert) {
        ...
        object.mixin(source);
        assert.strictEqual(object.a(array), __str_top__);
        assert.notOk(__str_top__ in _);
        assert.notOk(__str_top__ in _.prototype);
        reset(_);
    });
    ...
    QUnit.test('should accept a function `object`', function (assert) {
        ...
        assert.strictEqual(actual.value(), __str_top__);
    ...
    QUnit.test('should accept an `options`', function (assert) {
        ...
        function message(func, chain) {
            return (func === _ ? __str_top__ : __str_top__) + __str_top__ + (chain ? __str_top__ : __str_top__) + __str_top__;
        }
        lodashStable.each([
            _,
            Wrapper
            ...
            lodashStable.each([
                ...
                if (!isNpm) {
                    ...
                    if (options.chain) {
                        assert.strictEqual(actual.value(), __str_top__, message(func, __bool_top__));
                        assert.ok(actual instanceof func, message(func, __bool_top__));
                    } else {
                        assert.strictEqual(actual, __str_top__, message(func, __bool_top__));
                        assert.notOk(actual instanceof func, message(func, __bool_top__));
                    }
                    reset(func);
                } else {
                    skipAssert(assert, 2);
