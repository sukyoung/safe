...
#######################################
command="html"
options="-domprop -timeout 1200 -bottomdump -max-strset-size 20 -forin-unroll 1 -context-loop -locclone"
...
function runAnalysis {
	...
	echo "*" Analyzing"("$(($i + 1))/$WebsiteCount")"... $websiteName \"jsaf $command $options $domstatoption $websiteDir/$domstatout $websiteHTML\"
	`jsaf $command $options $domstatoption $websiteDir/$domstatout $websiteHTML > $websiteDir/$out`
