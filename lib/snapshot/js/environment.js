...
function ___saveDump(data, filename) {
    ...
    for (var k = 0; k < data1.length; k++) {
      ...
      // case 1 "#Property "  ->  #Property
      var line = data1[k];
      var pos = line.search('"#');
      var re = /"#.*"/;
      var result = re.exec(line);
      if (result !== null) {
        line = line.slice(0,pos) + line.slice(pos+1,pos+result[0].length-1) + line.slice(pos+result[0].length);
      }
      // case 2 "<value,descriptor option>"  ->  <value,descriptor option>
      var pos = line.search('"<');
      var re = /"<.*>"/;
      var result = re.exec(line);
      if (result !== null) {
      ...
      // case 3 "[[internal call]]"  ->  [[internal call]]
      var pos = line.search('"[[][[]');
      var re = /"[[][[].*:/;
      var result = re.exec(line);
      if (result !== null) {
        line = line.slice(0,pos) + line.slice(pos+1,pos+result[0].length-2) + line.slice(pos+result[0].length-1);
      }
      // case 4 "name": <property name,descriptor option>  ->  "name": <"property name",descroptor option>   irregular case of case2
      var pos = line.search('"name": <');
      var re = /"name": <.*,/;
      var result = re.exec(line);
      if (result !== null) {
        line = line.slice(0,pos+9) + '"' + line.slice(pos+9,pos+result[0].length-11) + '"' + line.slice(pos+result[0].length-11);
      }
      // case 5 "name": <"undefined",descriptor option>  ->  "name": <undefined,descriptor option>   irregular case of case 4
      var pos = line.search('"name": <"undefined",');
      var re = /"name": <"undefined",/;
      var result = re.exec(line);
      if (result !== null) {
        line = line.slice(0,pos+9) + line.slice(pos+10,pos+19) + line.slice(pos+20);
      }
      data2[k] = line;
    }
    data = data2.join('\n');
