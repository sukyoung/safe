This directory includes the following third_party software:

* Ant 1.6.5 (The Apache Software License Version 2.0)
  See http://ant.apache.org/bindownload.cgi

* ASTGen (The BSD License)
  See http://sourceforge.net/projects/astgen

* Commons IO (The Apache Software License Version 2.0)
  See http://commons.apache.org/io/

* Commons Lang (The Apache Software License Version 2.0)
  See http://commons.apache.org/lang/

* Deckard (The BSD License)
  See https://github.com/skyhover/Deckard

* JUnit 3.8.1 (The Common Public License Version 1.0)
  See http://prdownloads.sourceforge.net/junit/junit

* JLine 2.12 (The BSD License)
  See http://jline.github.io/jline2/

* Lift (The Apache Software License Version 2.0)
  See https://www.assembla.com/spaces/liftweb/wiki/JSON_Support
  
* PLT Utilities (The BSD License)
  See http://drjava.org

* PMD (The BSD License)
  See http://pmd.sourceforge.net

* Scala 2.9.0 (The BSD License)
  See http://www.scala-lang.org/

* Simian (The Simian Software License)
  See http://www.harukizaemon.com/simian

* Unicode 5.0.0 (The Unicode License)
  See http://unicode.org/versions/Unicode5.0.0/

* URIHandler(The Mozilla Public License Version 2.0)
  See https://github.com/mir597/urihandler

* WALA (The Eclipse Public License)
  See http://wala.sourceforge.net/wiki/index.php/Main_Page

* XTC (The GNU LESSER GENERAL PUBLIC LICENSE License)
  See http://cs.nyu.edu/rgrimm/xtc/
  
* Z3 (The Microsoft Research License Agreement)
  See http://z3.codeplex.com/
  
Details of the appropriate licenses are in their respective directories.
