...
\textit{{\sealed} state}, 2) performing \textit{{\sealed} execution}
on the {\sealed} state, and 3) converting a {\sealed} state
...
and lessen the modeling efforts by performing {\sealed} execution for opaque code.
...
Our experiment shows that $\tool$ is 7.81$\x$ faster than the
baseline static analyzer, and it improves the precision to
reduce failed assertions by 12.31\% on average by using {\sealed} execution for
22 opaque functions.
\end{abstract}
