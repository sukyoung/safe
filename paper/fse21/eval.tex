...
\inred{Except for 9 test cases, using dynamic shortcut did show speed up.}
...
\begin{figure}[t]
  ...
  \end{subfigure}
  \vspace*{-1em}
  \caption{Failed assertions of analysis without (no-DS) and with (DS) dynamic shortcuts for 199 \textit{original} tests (left) and 156 \textit{abstracted} tests (right)}
  \label{fig:precision}
  \vspace*{-1.5em}
\end{figure}
...
\inred{each circle denotes each test in heatmap form. The darker the circle is, the more tests it indicates.} The top line
...
Figure~\ref{fig:precision}(left) shows that dynamic shortcuts
\inred{completely removed the failed assertions for 24 tests, reducing the nuber of failed assertions by 92.79\% on average}.
For 156 abstracted tests that are analyzable by both analyzers,
Figure~\ref{fig:precision}(right) shows that dynamic shortcuts successfully cut down
