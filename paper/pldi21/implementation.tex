...
only when it terminates in a bound time $N$.  Since statically checking the
termination property is a difficult task, we simply perform sealed symbolic
execution with a pre-determined time limit.  When it runs over the time limit,
we treat it as a failure to convert the analysis element via $\asconverter$;
...
% sealed symbolic exeuction with a time limit and we use 5 minutes time
