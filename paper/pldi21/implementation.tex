...
creates revoked \jscode{Proxy} objects to represent abstract values via the
...
var z = x + 1;
\end{lstlisting}
The function creates a sealed symbol \jscode{r} as a revocable proxy object with a dummy
function object without any handlers. It then revokes the symbol by
\jscode{r.revoke()} not to allow any accesses of the symbol.  A revoked
object throws a \jscode{TypeError} exception when any of 13 pre-defined operations
...
program throws \jscode{TypeError} on line 8 because \jscode{x + 1} requires
...
symbol beyond the 13 operations provided by \jscode{Proxy}.
