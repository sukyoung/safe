...
/* Generates IRAst.scala, a Scala representation
 * of the JavaScript IR (IR.ast)
 */
public class ScalaIRGenerator extends CodeGenerator {
    public ScalaIRGenerator(ASTModel ast) {
        super(ast);
    }
    ...
    private StringBuilder allFieldsNoParens(NodeClass box, boolean firstPass) {
        ...
        // If the supertype has no definition, there is a bug in IR.ast or ASTGen.
        // If the defined supertype is not a class, there is a bug in IR.ast or ASTGen.
        try {
            buffer.append(fieldsNoParens((NodeClass) ast.typeForName(box.superClass()).unwrap(), firstPass));
        }
        catch (OptionUnwrapException e) {
            throw new RuntimeException("Missing supertype definition in IR.ast: " + box.superClass());
        }
        return buffer;
    }
    ...
    private boolean ignoreClass(String name) {
        ...
        /* TODO: this won't be needed once TemplateGap's are removed from IR.ast */
    ...
    /**
     * Given a PrintWriter, write out all Scala declarations for IR.ast.
     */
    ...
    private String copyright() {
        ...
        writer.println(sub("   @class FROM IR.ast */", "@class", this.getClass().getName()));
