...
/* Generates WIDLAst.scala, a Scala representation
 * of the JavaScript WIDL (WIDL.ast)
 */
public class ScalaWIDLGenerator extends CodeGenerator {
    public ScalaWIDLGenerator(ASTModel ast) {
        super(ast);
    }
    ...
    private StringBuilder allFieldsNoParens(NodeClass box, boolean firstPass) {
        ...
        // If the supertype has no definition, there is a bug in WIDL.ast or ASTGen.
        // If the defined supertype is not a class, there is a bug in WIDL.ast or ASTGen.
        try {
            buffer.append(fieldsNoParens((NodeClass) ast.typeForName(box.superClass()).unwrap(), firstPass));
        }
        catch (OptionUnwrapException e) {
            throw new RuntimeException("Missing supertype definition in WIDL.ast: " + box.superClass());
        }
        return buffer;
    }
    ...
    private boolean ignoreClass(String name) {
        ...
        /* TODO: this won't be needed once TemplateGap's are removed from WIDL.ast */
    ...
    /**
     * Given a PrintWriter, write out all Scala declarations for WIDL.ast.
     */
    private void generateBody(PrintWriter writer) {
        ...
        // Generate walker.
        writer.println();
        writer.println("trait WIDLWalker {");
    ...
    private String copyright() {
        ...
        writer.println(sub("   @class FROM WIDL.ast */", "@class", this.getClass().getName()));
