...
/* Generates RegExpAst.scala, a Scala representation
 * of the JavaScript RegExp (RegExp.ast)
 */
public class ScalaRegExpGenerator extends CodeGenerator {
    public ScalaRegExpGenerator(ASTModel ast) {
        super(ast);
    }
    ...
    private StringBuilder allFieldsNoParens(NodeClass box, boolean firstPass) {
        ...
        // If the supertype has no definition, there is a bug in RegExp.ast or ASTGen.
        // If the defined supertype is not a class, there is a bug in RegExp.ast or ASTGen.
        try {
            buffer.append(fieldsNoParens((NodeClass) ast.typeForName(box.superClass()).unwrap(), firstPass));
        }
        catch (OptionUnwrapException e) {
            throw new RuntimeException("Missing supertype definition in RegExp.ast: " + box.superClass());
        }
        return buffer;
    }
    ...
    private boolean ignoreClass(String name) {
        ...
        /* TODO: this won't be needed once TemplateGap's are removed from RegExp.ast */
    ...
    /**
     * Given a PrintWriter, write out all Scala declarations for RegExp.ast.
     */
    private void generateBody(PrintWriter writer) {
        ...
        // Generate walker.
        writer.println();
        writer.println("trait RegExpWalker {");
    ...
    private String copyright() {
        ...
        writer.println(sub("   @class FROM RegExp>.ast */", "@class", this.getClass().getName()));
