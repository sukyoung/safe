...
import kr.ac.kaist.safe.nodes.ASTNodeInfo;
import kr.ac.kaist.safe.nodes.Node;
import kr.ac.kaist.safe.nodes.NodeInfo;
import kr.ac.kaist.safe.nodes.ScopeBody;
...
abstract public class NodeReflection {
    ...
    public static final String NODES_PACKAGE_PREFIX = "kr.ac.kaist.safe.nodes.";
    ...
    // What happened here is that various forms of node reflection got merged into a
    // common superclass, and the needs of printing and unprintig were not exactly aligned.
    ...
    protected NodeReflection() {
        try {
            infoField = ASTNode.class.getDeclaredField("info");
    ...
    protected Node makeNodeFromSpan(String s, Class c, Span span)
        ...
        if (con != null) {
            ...
            for (int i = 0; i < con.getParameterTypes().length; i++) {
                if (con.getParameterTypes()[i].toString().equals("boolean"))
                    args[i] = false;
            }
            Node node = (Node)con.newInstance(args);
            if ( node instanceof ASTNode ) {
                infoField.set((ASTNode)node, NodeFactory.makeASTNodeInfo(span));
            }
            return node;
        }
    ...
    protected Class classFor(String class_name) {
        ...
        if (cl == null) {
            try {
                ...
                e.printStackTrace();
                error("Error reading node type " + class_name + ", class not found");
            } catch (SecurityException e) {
                e.printStackTrace();
                error("Error reading node type " + class_name);
            } catch (NoSuchMethodException e) {
                e.printStackTrace();
                error("Error reading node type " + class_name + ", missing constructor (Span)");
            } catch (NoSuchFieldException e) {
                e.printStackTrace();
                error("Error reading node type " + class_name + ", no such field");
            }
        }
        return cl;
    }
    ...
    private static void handleFields(Field[] fields, ArrayList<Field> fal) {
        for (int i = 0; i < fields.length; i++) {
            ...
            if ((f.getModifiers() &
                ...
                && ! f.getName().equals("hashCode")
                && ! f.getName().equals("hasHashCode") ) {
