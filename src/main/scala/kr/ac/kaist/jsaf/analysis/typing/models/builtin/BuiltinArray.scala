  ...
  def getPreSemanticMap(): Map[String, SemanticFun] = {
    Map(
      ...
      "Array.prototype.toLocaleString" -> (
        (sem: Semantics, h: Heap, ctx: Context, he: Heap, ctxe: Context, cp: ControlPoint, cfg: CFG, fun: String, args: CFGExpr) => {
          val PureLocalLoc = cfg.getPureLocal(cp)
          val v_this = h(PureLocalLoc)("@this")._2
        ...
        (sem: Semantics, h: Heap, ctx: Context, he: Heap, ctxe: Context, cp: ControlPoint, cfg: CFG, fun: String, args: CFGExpr) => {
          ...
          val o = AbsNumber.getUIntSingle(n_arglen) match {
            case Some(n_arg) => {
              ...
              val index = AbsNumber.alpha(0)
              val (obj_1, len) = elem_list.foldLeft((obj, index))((oi, elem) => {
                val lset_array = elem._2.filter((l) => AbsString.alpha("Array") <= h_1(l)("@class")._2._1._5)
                val lset_narray = elem._2.filter((l) => AbsString.alpha("Array") != h_1(l)("@class")._2._1._5)
                val v_narray = Value(elem._1, lset_narray)
                val o = oi._1
                val index = oi._2
                val (o_1, n_index_1) =
            ...
            case _ => n_arglen.getAbsCase match {
              ...
              case _ =>
                ...
                val lset_array = v_all._2.filter((l) => AbsString.alpha("Array") <= h_1(l)("@class")._2._1._5)
      ...
      ("Array.prototype.splice" -> (
        (sem: Semantics, h: Heap, ctx: Context, he: Heap, ctxe: Context, cp: ControlPoint, cfg: CFG, fun: String, args: CFGExpr) => {
          ...
          val (h_2, o) = (n_start.getSingle, n_count.getSingle) match {
            case (Some(start), Some(count)) =>
              lset_this.foldLeft((h_1, Obj.bottom))((_ho, l) => {
                val _h = _ho._1
                val _o = _ho._2
                          ...
                          if (n_start >= n_len)
                            ...
                            val (index, flag) = (k.toInt until n_len.toInt).foldLeft[(AbsNumber, Boolean)]((NumBot, false))((__nb, i) => {
                              if (__nb._2)
                                ...
                                val __n = __nb._1
                                Operator.bopSEq(v_search, PreHelper.Proto(h, l, AbsString.alpha(i.toString)))._1._3.getPair match {
                                  case (AbsTop, _) => (__n + AbsNumber.alpha(i), false)
                                  case (AbsBot, _) => (NumBot, true)
                                  case (AbsSingle, Some(true)) => (AbsNumber.alpha(i), true)
                                  case (AbsSingle, Some(false)) => (__n, false)
                        ...
                        case Some(n_start) => {
                          ...
                          val (index, flag) = (0 until k.toInt).foldLeft[(AbsNumber, Boolean)]((NumBot, false))((__nb, i) => {
                            if (__nb._2)
                              ...
                              val __n = __nb._1
                              val i_back = (k - i).toInt
                              Operator.bopSEq(v_search, PreHelper.Proto(h, l, AbsString.alpha(i_back.toString)))._1._3.getPair match {
                                case (AbsTop, _) => (__n + AbsNumber.alpha(i_back), false)
                                case (AbsBot, _) => (NumBot, true)
                                case (AbsSingle, Some(true)) => (AbsNumber.alpha(i_back), true)
                                case (AbsSingle, Some(false)) => (__n, false)
      ...
      "Array.prototype.reduce.call" -> (
        (sem: Semantics, h: Heap, ctx: Context, he: Heap, ctxe: Context, cp: ControlPoint, cfg: CFG, fun: String, args: CFGExpr) => {
          ...
          val temp = h_3(PureLocalLoc)("temp")._1._1
          val temp_2 = v_initial match {
            case Some(v) => v + temp + value
            case None => temp + value
          }
    ...
    Map(
      ("Array" -> (
        (h: Heap, ctx: Context, cfg: CFG, fun: String, args: CFGExpr, fid: FunctionId) => {
          ...
          val es1 = if (AbsNumber.isUIntOrBot(v_1._1._4)) ExceptionBot
                    else Set[Exception](RangeError)
          val es2 = AbsNumber.getUIntSingle(n_arglen) match {
            case Some(n) if n == 1 => es1
            case Some(n) if n != 1 => ExceptionBot
            case _ => n_arglen.getAbsCase match {
          ...
          val LP2 = AH.NewArrayObject_def.foldLeft(LPBot)((_lpset, prop) =>
            _lpset ++ set_addr.foldLeft(LPBot)((lp, a) => lp + (addrToLoc(cfg.getAPIAddress((fid, a), 0), Recent), prop)))
      ...
      "Array.prototype.toLocaleString" -> (
        (h: Heap, ctx: Context, cfg: CFG, fun: String, args: CFGExpr, fid: FunctionId) => {
          ...
          val lset_env = h(SinglePureLocalLoc)("@env")._2._2
          val set_addr = lset_env.foldLeft[Set[Address]](Set())((a, l) => a + locToAddr(l))
          val set_addr1 = set_addr.foldLeft(LocSetBot)((S, addr_env) => S + cfg.getAPIAddress((fid, addr_env), 0))
          val set_addr2 = set_addr.foldLeft(LocSetBot)((S, addr_env) => S + cfg.getAPIAddress((fid, addr_env), 1))
      ...
      ("Array.prototype.concat" -> (
        (h: Heap, ctx: Context, cfg: CFG, fun: String, args: CFGExpr, fid: FunctionId) => {
          ...
          val LP2 = o.getProps.foldLeft(LPBot)((lpset, x) =>
            if (!(x.take(1) == "@") && AbsString.alpha(x) <= NumStr)
              //lpset +  (l_r, x)
              lpset ++ set_addr.foldLeft(LPBot)((lp, a) => lp + (addrToLoc(cfg.getAPIAddress((fid, a), 0), Recent), x))
            //else lpset) + (l_r, Str_default_number)
            else lpset) ++ set_addr.foldLeft(LPBot)((lp, a) => lp + (addrToLoc(cfg.getAPIAddress((fid, a), 0), Recent), Str_default_number))
          LP1 ++ LP2 + (SinglePureLocalLoc, "@return")
        })),
      ("Array.prototype.join" -> (
        (h: Heap, ctx: Context, cfg: CFG, fun: String, args: CFGExpr, fid: FunctionId) => {
      ...
      ("Array.prototype.pop" -> (
        (h: Heap, ctx: Context, cfg: CFG, fun: String, args: CFGExpr, fid: FunctionId) => {
          ...
          val LP1 = lset_this.foldLeft(LPBot)((lpset, l) => {
            val n_len = Operator.ToUInt32(Helper.Proto(h, l, AbsString.alpha("length")))
            val LP = AbsNumber.getUIntSingle(n_len) match {
              case Some(n) if n == 0 =>
      ...
      ("Array.prototype.push" -> (
        (h: Heap, ctx: Context, cfg: CFG, fun: String, args: CFGExpr, fid: FunctionId) => {
          ...
          val LP1 = AbsNumber.getUIntSingle(n_arglen) match {
            case Some(n_arg) => {
              lset_this.foldLeft(LPBot)((lpset, l) => {
                val n_len = Operator.ToUInt32(Helper.Proto(h, l, AbsString.alpha("length")))
                val _LP = AbsNumber.getUIntSingle(n_len) match {
                  case Some(n) => {
                    val __LP1 =
      ...
      ("Array.prototype.slice" -> (
        (h: Heap, ctx: Context, cfg: CFG, fun: String, args: CFGExpr, fid: FunctionId) => {
          ...
          val LP2 = (n_start.getSingle, n_end.getSingle) match {
      ...
      ("Array.prototype.splice" -> (
        (h: Heap, ctx: Context, cfg: CFG, fun: String, args: CFGExpr, fid: FunctionId) => {
          ...
          val LP2 = (n_start.getSingle, n_count.getSingle) match {
            case (Some(start), Some(count)) =>
              lset_this.foldLeft(LPBot)((lpset, l) => {
                val n_len = Operator.ToUInt32(Helper.Proto(h, l, AbsString.alpha("length")))
                val _LP = AbsNumber.getUIntSingle(n_len) match {
                  case Some(n_len) => {
                    ...
                    val __LP2 = (0 until delCount).foldLeft(LPBot)((_lpset, i) =>
                      _lpset ++ set_addr.foldLeft(LPBot)((lp, a) => lp + (addrToLoc(cfg.getAPIAddress((fid, a), 0), Recent),i.toString)))
                    val __LP3 = AbsNumber.getUIntSingle(n_arglen) match {
                      case Some(n_arglen) => {
                        val add_count = n_arglen.toInt - 2
                        val move_start = start + count
                        if (add_count < count) {
            ...
            case _ =>
              if (n_start <= NumBot || n_count <= NumBot)
                ...
                lset_this.foldLeft(LPBot)((_lpset, l) => {
                  val n_len = Operator.ToUInt32(Helper.Proto(h, l, AbsString.alpha("length")))
                  val _LP = n_len.getAbsCase match {
                    case AbsBot => LPBot
                    case _ =>
                      //val __LP1 = AH.NewArrayObject_def.foldLeft(LPBot)((_lpset, p)=> _lpset + (l_r, p))
                      val __LP1 = AH.NewArrayObject_def.foldLeft(LPBot)((_lpset, p)=>
      ...
      ("Array.prototype.unshift" -> (
        (h: Heap, ctx: Context, cfg: CFG, fun: String, args: CFGExpr, fid: FunctionId) => {
          val lset_this = h(SinglePureLocalLoc)("@this")._2._2
          val n_arglen = Operator.ToUInt32(getArgValue(h, ctx, args, "length"))
          val LP = AbsNumber.getUIntSingle(n_arglen) match {
            case Some(n_arglen) => {
              lset_this.foldLeft(LPBot)((lpset, l) => {
                val n_len = Operator.ToUInt32(Helper.Proto(h, l, AbsString.alpha("length")))
                val _LP = AbsNumber.getUIntSingle(n_len) match {
      ...
      "Array.prototype.reduce.init" -> (
        (h: Heap, ctx: Context, cfg: CFG, fun: String, args: CFGExpr, fid: FunctionId) => {
          ...
          if (set_addr.size > 1) throw new InternalError("API heap allocation: Size of env address is " + set_addr.size)
          val addr_env = set_addr.head
          val addr1 = cfg.getAPIAddress((fid, addr_env), 0)
    ...
    Map(
          ...
          val LP3 = AbsNumber.getUIntSingle(n_arglen) match {
            case Some(n) =>
              ...
              (0 until n.toInt).foldLeft(LPBot)((_lpset, i) =>
                set_addr.foldLeft(LPBot)((lp, a) => lp + (addrToLoc(cfg.getAPIAddress((fid, a), 0), Recent), i.toString)))
            case _ => n_arglen.getAbsCase match {
              case AbsBot => LPBot
              case _ =>
                //AH.absPair(h, l_r, NumStr)
                set_addr.foldLeft(LPBot)((lp, a) => lp ++ AH.absPair(h, addrToLoc(cfg.getAPIAddress((fid, a), 0), Recent), NumStr))
            }
          }
      ...
      ("Array.prototype.toString" -> (
        (h: Heap, ctx: Context, cfg: CFG, fun: String, args: CFGExpr, fid: FunctionId) => {
          ...
          val LP2 = AbsNumber.getUIntSingle(n_len) match {
            ...
            case Some(n) if n > 0 => {
              ...
              val _LP2 = (1 until n.toInt).foldLeft(LPBot)((lpset, i) =>{
                lpset ++ lset_this.foldLeft(LPBot)((_lpset, l) => _lpset ++ AH.Proto_use(h, l, AbsString.alpha(i.toString)) ++ AH.toPrimitive_use(h, h(l)(i.toString)._1._1))
              })
            ...
            case _ => n_len.getAbsCase match {
              case _ if AbsNumber.isUIntAll(n_len) =>
                lset_this.foldLeft(LPBot)((lpset, l) => lpset  ++ AH.Proto_use(h, l, NumStr) ++ AH.toPrimitive_use(h, h(l)(NumStr)._1._1))
              case _ => LPBot
            }
          }
      ...
      ("Array.prototype.concat" -> (
        (h: Heap, ctx: Context, cfg: CFG, fun: String, args: CFGExpr, fid: FunctionId) => {
          ...
          val LP3_arg = AbsNumber.getUIntSingle(n_arglen) match {
            case Some(n_arg) => {
              (0 until n_arg.toInt).foldLeft(LPBot)((lpset, i) => {
      ...
      ("Array.prototype.join" -> (
        (h: Heap, ctx: Context, cfg: CFG, fun: String, args: CFGExpr, fid: FunctionId) => {
          ...
          val LP3 = AbsNumber.getUIntSingle(n_len) match {
            case Some(n) if n > 0 => {
      ...
      ("Array.prototype.pop" -> (
        (h: Heap, ctx: Context, cfg: CFG, fun: String, args: CFGExpr, fid: FunctionId) => {
          ...
          val LP2 =  lset_this.foldLeft(LPBot)((lpset, l) => {
            ...
            val _LP2 = AbsNumber.getUIntSingle(n_len) match {
              case Some(n) if n == 0 =>
      ...
      ("Array.prototype.slice" -> (
        (h: Heap, ctx: Context, cfg: CFG, fun: String, args: CFGExpr, fid: FunctionId) => {
          ...
          val LP3 = (n_start.getSingle, n_end.getSingle) match {
            case (Some(start), Some(end)) =>
              lset_this.foldLeft(LPBot)((lpset, l) => {
                val n_len = Operator.ToUInt32(Helper.Proto(h, l, AbsString.alpha("length")))
                val _LP1 = AH.Proto_use(h, l, AbsString.alpha("length"))
            ...
            case _ =>
              if (n_start <= NumBot || n_end <= NumBot)
                ...
                lset_this.foldLeft(LPBot)((lpset, l) => {
                  val n_len = Operator.ToUInt32(Helper.Proto(h, l, AbsString.alpha("length")))
                  val _LP1 = AH.Proto_use(h, l, AbsString.alpha("length"))
                  val _LP2 = n_len.getAbsCase match {
                    case AbsBot => LPBot
                    case _ => AH.Proto_use(h, l, NumStr)
                  }
                  lpset ++ _LP1 ++ _LP2
      ...
      ("Array.prototype.splice" -> (
        (h: Heap, ctx: Context, cfg: CFG, fun: String, args: CFGExpr, fid: FunctionId) => {
          ...
          val LP3 = (n_start.getSingle, n_count.getSingle) match {
            case (Some(start), Some(count)) =>
              lset_this.foldLeft(LPBot)((lpset, l) => {
                val n_len = Operator.ToUInt32(Helper.Proto(h, l, AbsString.alpha("length")))
                val _LP1 = AH.Proto_use(h, l, AbsString.alpha("length"))
                val _LP2 = AbsNumber.getUIntSingle(n_len) match {
                  case Some(n_len) => {
                    ...
                    val __LP2 = AbsNumber.getUIntSingle(n_arglen) match {
                      case Some(n_arglen) => {
                        val add_count = n_arglen.toInt - 2
                        val move_start = start + count
                        if (add_count < count) {
          ...
          val LP2 = AbsNumber.getUIntSingle(n_arglen) match {
            case Some(n_arglen) => {
              lset_this.foldLeft(LPBot)((lpset, l) => {
                ...
                val lpset2 = AbsNumber.getUIntSingle(n_len) match {
                  case Some(k) => {
                ...
                val lpset2 = AbsNumber.getUIntSingle(n_len) match {
                  case Some(n_len) => {
                    if (n_len == 0)
                      ...
                      val __LP2 = start.getSingle match {
                        case Some(n_start) =>
                          val k =
                          if (n_start >= 0) min(n_start, n_len-1)
                          else (n_len - abs(n_start))
