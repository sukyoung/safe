...
object HTMLUListElement extends DOM {
  private val name = "HTMLUListElement"
  ...
  def getInsList(compact: PropValue, ttype: PropValue, xpath: PropValue): List[(String, PropValue)] = List(
    ("@class",    PropValue(AbsString.alpha("Object"))),
    ("@proto",    PropValue(ObjectValue(loc_proto, F, F, F))),
    ("@extensible", PropValue(BoolTrue)),
    // DOM Level 1
  ...
  override def default_getInsList(): List[(String, PropValue)] = { 
    ...
    HTMLElement.default_getInsList ::: 
      getInsList(compact, ttype, xpath)
  }
}
