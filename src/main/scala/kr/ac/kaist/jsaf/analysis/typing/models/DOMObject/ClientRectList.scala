...
object ClientRectList extends DOM {
  ...
  private val prop_proto: List[(String, AbsProperty)] = List(
    ...
    ("item",   AbsBuiltinFunc("ClientRectList.item", 1))
  )
  ...
  private val prop_ins: List[(String, AbsProperty)] = List(
    ...
    (Str_default_number, AbsConstValue(PropValue(ObjectValue(Value(ClientRect.loc_ins), T, T, T))))
  )
