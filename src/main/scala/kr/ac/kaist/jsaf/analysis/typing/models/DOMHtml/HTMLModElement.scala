...
object HTMLModElement extends DOM {
  private val name = "HTMLModElement"
  ...
  private val prop_ins: List[(String, AbsProperty)] = 
      ...
      ("@extensible", AbsConstValue(PropValue(BoolTrue))),
      // DOM Level 1
      ("cite", AbsConstValue(PropValue(ObjectValue(StrTop, T, T, T)))),
      ("dateTime", AbsConstValue(PropValue(ObjectValue(StrTop, T, T, T))))
    )
  /* prorotype */
  ...
  override def getInsList(node: Node): List[(String, PropValue)] = node match {
    case e: Element => 
      ...
      // DOM Level 1
      ("cite", PropValue(ObjectValue(AbsString.alpha(e.getAttribute("cite")), T, T, T))),
      ("dateTime", PropValue(ObjectValue(AbsString.alpha(e.getAttribute("dateTime")), T, T, T))))
    case _ => {
  ...
  def getInsList(cite: PropValue, dateTime: PropValue, xpath: PropValue): List[(String, PropValue)] = List(
    ("@class",    PropValue(AbsString.alpha("Object"))),
    ("@proto",    PropValue(ObjectValue(loc_proto, F, F, F))),
    ("@extensible", PropValue(BoolTrue)),
    // DOM Level 1
  ...
  override def default_getInsList(): List[(String, PropValue)] = { 
    val cite = PropValue(ObjectValue(AbsString.alpha(""), T, T, T))
    val dataTime = PropValue(ObjectValue(AbsString.alpha(""), T, T, T))
    val xpath = PropValue(ObjectValue(AbsString.alpha(""), F, F, F))
    // This object has all properties of the HTMLElement object 
    HTMLElement.default_getInsList ::: getInsList(cite, dataTime, xpath)
  }
