...
object HTMLIsIndexElement extends DOM {
  private val name = "HTMLIsIndexElement"
  ...
  private val prop_ins: List[(String, AbsProperty)] = 
      ...
      ("prompt", AbsConstValue(PropValue(ObjectValue(StrTop, T, T, T))))
    )
  /* prorotype */
  ...
  override def getInsList(node: Node): List[(String, PropValue)] = node match {
    case e: Element => 
      ...
      // DOM Level 1
      ("prompt",   PropValue(ObjectValue(AbsString.alpha(e.getAttribute("prompt")), T, T, T))))
  ...
  def getInsList(prompt: PropValue, xpath: PropValue): List[(String, PropValue)] = List(
    ("@class",    PropValue(AbsString.alpha("Object"))),
    ("@proto",    PropValue(ObjectValue(loc_proto, F, F, F))),
    ("@extensible", PropValue(BoolTrue)),
    // DOM Level 1
  ...
  override def default_getInsList(): List[(String, PropValue)] = {
    ...
    // This object has all properties of the HTMLElement object 
    HTMLElement.default_getInsList ::: getInsList(PropValue(ObjectValue(AbsString.alpha(""), T, T, T)), xpath)
  } 
}
