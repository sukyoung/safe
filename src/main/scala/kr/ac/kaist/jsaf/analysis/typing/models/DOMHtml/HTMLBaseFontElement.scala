...
object HTMLBaseFontElement extends DOM {
  private val name = "HTMLBaseFontElement"
  ...
  override def getInsList(node: Node): List[(String, PropValue)] = node match {
    case e: Element => 
      ...
      // DOM Level 1
      ("color", PropValue(ObjectValue(AbsString.alpha(e.getAttribute("color")), T, T, T))),
      ("face", PropValue(ObjectValue(AbsString.alpha(e.getAttribute("face")), T, T, T))),
