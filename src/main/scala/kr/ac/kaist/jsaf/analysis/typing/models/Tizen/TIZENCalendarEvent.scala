...
object TIZENCalendarEvent extends Tizen {
  val name = "CalendarEvent"
  ...
  private val prop_proto: List[(String, AbsProperty)] = List(
    ...
    ("@extensible", AbsConstValue(PropValue(T))),
    ("expandRecurrence", AbsBuiltinFunc("tizen.CalendarEvent.expandRecurrence", 4))
  )
          ...
          val (h_5, es_1) = AbsNumber.getUIntSingle(n_arglen) match {
            ...
            case Some(n) if n == 1 =>
              ...
              val (obj, es_1) = v._2.foldLeft((o_new, TizenHelper.TizenExceptionBot))((_o, l) => {
                ...
                val o_1 =
                  if (b_1._1._3 <= T)
                    _o._1.update("endDate", PropValue(ObjectValue(Value(v_1._2), T, T, T)))
                  else _o._1.update("endDate", PropValue(ObjectValue(Value(UndefTop), T, T, T)))
                val es_3 =
                  if (v_2._1._1 </ UndefTop && v_2._1._5 </ AbsString.alpha("BUSY") && v_2._1._5 </ AbsString.alpha("FREE"))
                    Set[WebAPIException](TypeMismatchError)
                  else TizenHelper.TizenExceptionBot
                val o_2 =
                ...
                val o_3 =
                  if (b_2._1._3 <= T)
                    _o._1.update("recurrenceRule", PropValue(ObjectValue(Value(v_3._2), T, T, T)))
                  else _o._1.update("recurrenceRule", PropValue(ObjectValue(Value(UndefTop), T, T, T)))
                val es_6 =
                ...
                (o_1 + o_2 + o_3 + o_4 + o_5 + o_6 + o_7 + o_8 + o_9 + o_10 + o_11 + o_12 + o_13 + o_14 + o_15 + o_16 + o_17,
