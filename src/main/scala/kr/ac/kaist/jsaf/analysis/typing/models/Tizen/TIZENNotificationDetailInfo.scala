...
object TIZENNotificationDetailInfo extends Tizen {
  private val name = "NotificationDatailInfo"
  ...
  private val prop_cons: List[(String, AbsProperty)] = List(
    ...
    ("@construct",               AbsInternalFunc("tizen.NotificationDetailInfo.constructor")),
  ...
  override def getSemanticMap(): Map[String, SemanticFun] = {
    Map(
       ("tizen.NotificationDetailInfo.constructor" -> (
         (sem: Semantics, h: Heap, ctx: Context, he: Heap, ctxe: Context, cp: ControlPoint, cfg: CFG, fun: String, args: CFGExpr) => {
           ...
           val o_new = ObjEmpty.
             ...
             update("@proto", PropValue(ObjectValue(Value(TIZENNotificationDetailInfo.loc_proto), F, F, F))).
             update("@extensible", PropValue(T))

           val (h_2, es_2) = AbsNumber.getUIntSingle(n_arglen) match {
             case Some(n) if n == 1 =>
               val o_new2 = o_new.
                 update("mainText", PropValue(ObjectValue(Value(v_1._1._5), T, T, T))).
                 update("subText", PropValue(ObjectValue(Value(UndefTop), T, T, T)))
             ...
             case Some(n) if n >= 2 =>
               ...
               val o_new2 = o_new.
                 update("mainText", PropValue(ObjectValue(Value(v_1._1._5), T, T, T))).
                 update("subText", PropValue(ObjectValue(Value(v_2._1._5), T, T, T)))
               val h_2 = lset_this.foldLeft(h)((_h, l) => _h.update(l, o_new2))
               (h_2, es_2)
