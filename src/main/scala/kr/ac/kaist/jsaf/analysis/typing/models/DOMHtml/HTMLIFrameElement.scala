...
object HTMLIFrameElement extends DOM {
  ...
  private val prop_ins: List[(String, AbsProperty)] = 
      ...
      ("contentWindow", AbsConstValue(PropValue(ObjectValue(Value(DOMWindow.loc_ins), T, T, T))))
    )
  /* prorotype */
  ...
  override def getInsList(node: Node): List[(String, PropValue)] = node match {
    case e: Element => 
      ...
      // 'contentWindow', 'contentDocument' are updated in DOMHelper.modelNode           
    )
  ...
  def getInsList(align: PropValue, frameBorder: PropValue, height: PropValue, longDesc: PropValue, marginHeight: PropValue,
                 ...
                 contentWindow: PropValue, xpath: PropValue): List[(String, PropValue)] = List(
    ("@class",    PropValue(AbsString.alpha("Object"))),
    ("@proto",    PropValue(ObjectValue(loc_proto, F, F, F))),
    ("@extensible", PropValue(BoolTrue)),
    // DOM Level 1
  ...
  override def default_getInsList(): List[(String, PropValue)] = {    
    ...
    HTMLElement.default_getInsList ::: 
      getInsList(align, frameBorder, height, longDesc, marginHeight, marginWidth, name, scrolling, src, width, contentWindow, xpath)
  }
