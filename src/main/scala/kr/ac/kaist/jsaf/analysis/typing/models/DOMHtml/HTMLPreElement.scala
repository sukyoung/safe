...
object HTMLPreElement extends DOM {
  private val name = "HTMLPreElement"
  ...
  override def getInsList(node: Node): List[(String, PropValue)] = node match {
    case e: Element => 
      ...
      // DOM Level 1
      ("cite",  PropValue(ObjectValue(Helper.toNumber(PValue(AbsString.alpha(e.getAttribute("cite")))), T, T, T))))
    case _ => {
