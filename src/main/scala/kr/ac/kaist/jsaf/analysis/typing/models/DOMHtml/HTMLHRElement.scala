...
object HTMLHRElement extends DOM {
  ...
  private val prop_ins: List[(String, AbsProperty)] = 
      ...
      ("noShade", AbsConstValue(PropValue(ObjectValue(BoolTop, T, T, T)))),
  ...
  def getInsList(align: PropValue, noShade: PropValue, size: PropValue, width: PropValue, xpath: PropValue): List[(String, PropValue)] = List(
    ("@class",    PropValue(AbsString.alpha("Object"))),
    ("@proto",    PropValue(ObjectValue(loc_proto, F, F, F))),
    ("@extensible", PropValue(BoolTrue)),
    // DOM Level 1
  ...
  override def default_getInsList(): List[(String, PropValue)] = { 
    ...
    HTMLElement.default_getInsList ::: 
      getInsList(align, noShade, size, width, xpath)
  }
