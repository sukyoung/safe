...
object HTMLScriptElement extends DOM {
  ...
  private val prop_ins: List[(String, AbsProperty)] = 
      ...
      ("event", AbsConstValue(PropValue(ObjectValue(StrTop, T, T, T)))),
      ("charset", AbsConstValue(PropValue(ObjectValue(StrTop, T, T, T)))),
      ("defer", AbsConstValue(PropValue(ObjectValue(BoolTop, T, T, T)))),
  ...
  def getInsList(text: PropValue, htmlFor: PropValue, event: PropValue, charset: PropValue, defer: PropValue, 
                 src: PropValue, ttype: PropValue, xpath: PropValue): List[(String, PropValue)] = List(
    ("@class",    PropValue(AbsString.alpha("Object"))),
    ("@proto",    PropValue(ObjectValue(loc_proto, F, F, F))),
    ("@extensible", PropValue(BoolTrue)),
    // DOM Level 1
  ...
  override def default_getInsList(): List[(String, PropValue)] = { 
    ...
    HTMLElement.default_getInsList ::: 
      getInsList(text, htmlFor, event, charset, defer, src, ttype, xpath)
  }
