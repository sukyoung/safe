...
object HTMLFrameSetElement extends DOM {
  private val name = "HTMLFrameSetElement"
  ...
  override def getInsList(node: Node): List[(String, PropValue)] = node match {
    case e: Element => 
      ...
      // DOM Level 1
      ("cols", PropValue(ObjectValue(AbsString.alpha(e.getAttribute("cols")), T, T, T))),
      ("rows", PropValue(ObjectValue(AbsString.alpha(e.getAttribute("rows")), T, T, T))))
    case _ => {
