...
object HTMLHtmlElement extends DOM {
  ...
  private val prop_ins: List[(String, AbsProperty)] = 
      ...
      ("@extensible", AbsConstValue(PropValue(BoolTrue))),
      // DOM Level 1
      ("clientWidth", AbsConstValue(PropValue(ObjectValue(StrTop, T, T, T)))),
      ("clientHeight", AbsConstValue(PropValue(ObjectValue(StrTop, T, T, T)))),
      ("version", AbsConstValue(PropValue(ObjectValue(StrTop, T, T, T))))
    )
  ...
  def getInsList(version: PropValue, xpath: PropValue): List[(String, PropValue)] = List(
    ("@class",    PropValue(AbsString.alpha("Object"))),
    ("@proto",    PropValue(ObjectValue(loc_proto, F, F, F))),
    ("@extensible", PropValue(BoolTrue)),
    // DOM Level 1
