...
object HTMLButtonElement extends DOM {
  ...
  private val prop_ins: List[(String, AbsProperty)] = 
      ...
      ("@extensible", AbsConstValue(PropValue(BoolTrue))),
      // DOM Level 1
      ("form", AbsConstValue(PropValue(ObjectValue(Value(HTMLFormElement.loc_ins), T, T, T)))),
  ...
  def getInsList(accessKey: PropValue, disabled: PropValue, name: PropValue, 
                 tabIndex: PropValue, ttype: PropValue, value: PropValue, form: PropValue, xpath: PropValue): List[(String, PropValue)] = List(
    ("@class",    PropValue(AbsString.alpha("Object"))),
    ("@proto",    PropValue(ObjectValue(loc_proto, F, F, F))),
    ("@extensible", PropValue(BoolTrue)),
    // DOM Level 1
  ...
  override def default_getInsList(): List[(String, PropValue)] = { 
    ...
    HTMLElement.default_getInsList ::: 
      getInsList(accessKey, disabled, name,tabIndex, ttype, value, form, xpath)
  }
