...
object HTMLHeadElement extends DOM {
  private val name = "HTMLHeadElement"
  ...
  private val prop_ins: List[(String, AbsProperty)] = 
      ...
      ("@extensible", AbsConstValue(PropValue(BoolTrue))),
      // DOM Level 1
      ("profile", AbsConstValue(PropValue(ObjectValue(StrTop, T, T, T))))
    )
  ...
  override def getInsList(node: Node): List[(String, PropValue)] = node match {
    case e: Element => 
      ...
      // DOM Level 1
      ("profile",     PropValue(ObjectValue(AbsString.alpha(e.getAttribute("profile")), T, T, T))))
    case _ => {
  ...
  def getInsList(profile: PropValue, xpath: PropValue): List[(String, PropValue)] = List(
    ("@class",    PropValue(AbsString.alpha("Object"))),
    ("@proto",    PropValue(ObjectValue(loc_proto, F, F, F))),
    ("@extensible", PropValue(BoolTrue)),
    // DOM Level 1
  ...
  override def default_getInsList(): List[(String, PropValue)] = {   
    val xpath = PropValue(ObjectValue(AbsString.alpha(""), F, T, T))
    // This object has all properties of the HTMLElement object 
    HTMLElement.default_getInsList ::: getInsList(PropValue(ObjectValue(AbsString.alpha(""), F, F, F)), xpath)
  }
}
