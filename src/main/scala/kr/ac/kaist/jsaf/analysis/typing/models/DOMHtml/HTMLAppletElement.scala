...
object HTMLAppletElement extends DOM {
  ...
  private val prop_ins: List[(String, AbsProperty)] = 
      ...
      ("object", AbsConstValue(PropValue(ObjectValue(StrTop, T, T, T)))),
  ...
  def getInsList(align: PropValue, alt: PropValue, archive: PropValue, code: PropValue, codeBase: PropValue, 
                 height: PropValue, name: PropValue, width: PropValue, hspace: PropValue, oobject: PropValue, vspace: PropValue, xpath: PropValue): List[(String, PropValue)] = List(
    ("@class",    PropValue(AbsString.alpha("Object"))),
    ("@proto",    PropValue(ObjectValue(loc_proto, F, F, F))),
    ("@extensible", PropValue(BoolTrue)),
    // DOM Level 1
  ...
  override def default_getInsList(): List[(String, PropValue)] = {    
    ...
    HTMLElement.default_getInsList ::: 
      getInsList(align, alt, archive, code, codeBase, height, name, width, hspace, oobject, vspace, xpath)
  }
}
