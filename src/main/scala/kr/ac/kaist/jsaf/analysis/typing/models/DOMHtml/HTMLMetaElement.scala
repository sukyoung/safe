...
object HTMLMetaElement extends DOM {
  ...
  private val prop_ins: List[(String, AbsProperty)] = 
      ...
      ("@extensible", AbsConstValue(PropValue(BoolTrue))),
      // DOM Level 1
      ("content", AbsConstValue(PropValue(ObjectValue(StrTop, T, T, T)))),
      ("httpEquiv", AbsConstValue(PropValue(ObjectValue(StrTop, T, T, T)))),
      ("name", AbsConstValue(PropValue(ObjectValue(StrTop, T, T, T)))),
      ("scheme", AbsConstValue(PropValue(ObjectValue(StrTop, T, T, T))))
    )
  /* prorotype */
  ...
  def getInsList(content: PropValue, httpEquiv: PropValue, name: PropValue, scheme: PropValue, xpath: PropValue): List[(String, PropValue)] = List(
    ("@class",    PropValue(AbsString.alpha("Object"))),
    ("@proto",    PropValue(ObjectValue(loc_proto, F, F, F))),
    ("@extensible", PropValue(BoolTrue)),
    // DOM Level 1
  ...
  override def default_getInsList(): List[(String, PropValue)] = { 
    ...
    HTMLElement.default_getInsList ::: 
      getInsList(content, httpEquiv, name, scheme, xpath)
  }
