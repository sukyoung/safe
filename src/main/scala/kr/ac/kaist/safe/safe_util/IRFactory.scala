...
object IRFactory {
  ...
  def makeFunctional(info: IRNodeInfo, fromSource: Boolean, ast: ASTNode,
    ...
    new IRFunctional(info, fromSource, name, params, args, fds, vds, body)
  ...
  def makeRoot(fromSource: Boolean, ast: ASTNode, span: Span, fds: List[IRFunDecl], vds: List[IRVarStmt],
    ...
    new IRRoot(makeNodeInfo(fromSource, span), fds, vds, irs)
  ...
  def getBody(ast: Option[ASTNode]): String = ast match {
    case Some(FunExpr(_, Functional(_, _, _, _, _, _, bodyS))) => bodyS
    case Some(FunDecl(_, Functional(_, _, _, _, _, _, bodyS), _)) => bodyS
    case Some(GetProp(_, _, Functional(_, _, _, _, _, _, bodyS))) => bodyS
    case Some(SetProp(_, _, Functional(_, _, _, _, _, _, bodyS))) => bodyS
  ...
  def makeFunExpr(fromSource: Boolean, ast: ASTNode,
    ...
    val info = makeNodeInfo(fromSource, span)
  ...
  def makeExprStmt(ast: ASTNode, span: Span, lhs: IRId, right: IRExpr, isRef: Boolean): IRExprStmt =
    new IRExprStmt(makeNodeInfo(false, span), lhs, right, isRef)
  ...
  def makeTry(fromSource: Boolean, ast: ASTNode, span: Span,
    ...
    new IRTry(makeNodeInfo(fromSource, span), body, name, catchB, finallyB)

  def makeStore(fromSource: Boolean, ast: ASTNode, span: Span, obj: IRId, index: IRExpr, rhs: IRExpr): IRStore =
    new IRStore(makeNodeInfo(fromSource, span), obj, index, rhs)
  ...
  def makeSeq(ast: ASTNode, span: Span, stmts: List[IRStmt]): IRSeq =
    new IRSeq(makeNodeInfo(false, span), stmts)
  ...
  def makeStmtUnit(ast: ASTNode, span: Span, stmts: List[IRStmt]): IRStmtUnit =
    new IRStmtUnit(makeNodeInfo(true, span), stmts)
  ...
  def makeField(fromSource: Boolean, ast: ASTNode, span: Span, prop: IRId, expr: IRExpr): IRField =
    new IRField(makeNodeInfo(fromSource, span), prop, expr)

  val defaultSpan = NU.makeSpan("Default span for internally generated nodes")
  val defaultInfo = new IRNodeInfo(defaultSpan, false)
  def trueInfo(ast: ASTNode): IRNodeInfo = new IRNodeInfo(ast.info.span, true)
  ...
  def makeTId(fromSource: Boolean, ast: ASTNode, span: Span, originalName: String, uniqueName: String,
    ...
    new IRTmpId(makeNodeInfo(fromSource, span), originalName, uniqueName, isGlobal)
  ...
  def makeNoOp(ast: ASTNode, span: Span, desc: String): IRNoOp =
    new IRNoOp(makeNodeInfo(false, span), desc)
}
