...
// JsonLoad phase
case object JsonLoad extends PhaseObj[Unit, JsonLoadConfig, (CFG, Worklist, Semantics, TracePartition, HeapBuildConfig)] {
  ...
  ): Try[(CFG, Worklist, Semantics, TracePartition, HeapBuildConfig)] = safeConfig.fileNames match {
        ...
        case JSONFile =>
          try {
            ...
            source.mkString.parseJson match {
              case JsArray(Vector(cfgJson, worklistJson, semJson, heapConfigJson)) => {
                ...
                Success((cfg, worklist, sem, sens.initTP, heapConfig))
              }
              case _ => {
                source.close
