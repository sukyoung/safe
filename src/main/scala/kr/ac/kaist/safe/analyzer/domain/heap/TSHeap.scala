...
object TSHeap extends AbsHeapUtil {
  ...
  case class HeapMap(
    ...
    def totalMap: Map[Loc, AbsObject] = globalMap ++ map
  }
  ...
  sealed abstract class Dom extends AbsHeap {
    ...
    private def buildString(filter: Loc => Boolean): String = this match {
      ...
      case heap @ HeapMap(_) =>
        ...
        val sortedSeq =
          heap.totalMap.toSeq.filter { case (loc, _) => filter(loc) }
    ...
    def get(loc: Loc): AbsObject = this match {
      ...
      case heap @ HeapMap(_) => heap.totalMap.get(loc) match {
        case Some(obj) => obj
    ...
    private def weakUpdated(heap: HeapMap, loc: Loc, newObj: AbsObject): HeapMap = {
      HeapMap(heap.totalMap.get(loc) match {
        case Some(oldObj) => heap.map.updated(loc, oldObj + newObj)
        case None => heap.map.updated(loc, newObj)
      })
    }
    ...
    def weakUpdate(loc: Loc, obj: AbsObject): AbsHeap = this match {
      case Top => Top
      case Bot => Bot
      case heap @ HeapMap(_) => weakUpdated(heap, loc, obj)
    }
    ...
    def domIn(loc: Loc): Boolean = this match {
      case Top => true
      case Bot => false
      case heap @ HeapMap(_) => heap.totalMap.contains(loc)
    }
    ...
    def toStringLoc(loc: Loc): Option[String] = this match {
      ...
      case Bot => None
      case heap @ HeapMap(_) => heap.totalMap.get(loc).map(toStringLoc(loc, _, isConcrete(loc)))
    }
    ...
    def getMap: Option[Map[Loc, AbsObject]] = this match {
      case Top => None
      case Bot => None
      case heap @ HeapMap(_) => Some(heap.totalMap)
    }
