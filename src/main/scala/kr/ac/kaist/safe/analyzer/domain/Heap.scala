...
class DHeap(val map: Map[Loc, AbsObject]) extends Heap {
  ...
  def lookupGlobal(x: String): (AbsValue, Set[Exception]) = {
    ...
    if (this domIn BuiltinGlobal.loc) {
      val globalObj = this.get(BuiltinGlobal.loc)
      val v1 =
        if (AbsBool.True <= (globalObj contains x))
          globalObj(x).value
        else
          valueBot
      val protoLocSet = globalObj(IPrototype).value.locset
      val (v2, excSet) =
        if (AbsBool.False <= (globalObj contains x)) {
          ...
          val v3 = protoLocSet.foldLeft(valueBot)((tmpVal, protoLoc) => {
            if (AbsBool.True <= hasProperty(protoLoc, AbsString(x))) {
              tmpVal + this.get(protoLoc).Get(x, this)
            } else {
              tmpVal
            }
          })
  ...
  def lookupBaseGlobal(x: String): AbsLoc = {
    val globalObj = this.get(BuiltinGlobal.loc)
    val isDomIn = globalObj contains x
    val locSet1 =
  ...
  def varStoreGlobal(x: String, value: AbsValue): Heap = {
    val globalLoc = BuiltinGlobal.loc
    val obj = this.get(globalLoc)
    val h1 =
      if (AbsBool.False <= (obj contains x))
        this.propStore(globalLoc, AbsString(x), value)
      else Heap.Bot
    val h2 =
      if (AbsBool.True <= (obj contains x)) {
        val oldObjVal = obj(x)
        val newObjVal = oldObjVal.copyWith(value)
        this.update(globalLoc, obj.update(x, newObjVal))
      } else Heap.Bot
    h1 + h2
  }
