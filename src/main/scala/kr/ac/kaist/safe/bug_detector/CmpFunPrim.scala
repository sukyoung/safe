...
object CmpFunPrim extends BugChecker {
  ...
  def cmpFunPrim(expr: CFGExpr, l: CFGExpr, r: CFGExpr): String = {
    ...
    val lexpr = l.ir.ast.toString(0)
    val rexpr = r.ir.ast.toString(0)
    s"$span\n    [Warning] Comparison between function and primitive value: $lexpr and $rexpr."
  }
  ...
  def isPrim(v: AbsValue): Boolean = {
    ...
    val numT: AbsValue = AbsNum.Top
    val boolT: AbsValue = AbsBool.Top
    val strT: AbsValue = AbsStr.Top

    val prims = numT ⊔ boolT ⊔ strT

    (v ⊓ prims) !⊑ AbsValue.Bot
  }
