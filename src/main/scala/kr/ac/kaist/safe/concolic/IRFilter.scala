...
class IRFilter(program: IRRoot) extends IRGeneralWalker[Boolean] {

  lazy val result: IRRoot = doit

  def join(args: Boolean*): Boolean = args.exists((arg) => arg)

  private def doit: IRRoot = {
  ...
  override def walk(node: IRStmt): Boolean = node match {
    ...
    case IRCall(info, lhs, fun, thisB, args) =>
      false
    case IRInternalCall(info, lhs, fun, args) =>
      false
    ...
    case IRLabelStmt(info, label, stmt) =>
      walk(stmt)
    case IRVarStmt(info, lhs, fromParam) =>
      true
