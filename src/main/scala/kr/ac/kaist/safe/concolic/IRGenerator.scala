    ...
    case n@New(info, lhs) =>
      ...
      ((ssl:+toObject(lhs, fun, rl))++args++
        List(IF.makeLoadStmt(false, e, proto, fun, IF.makeString("prototype", n)),
          ...
          IF.makeIf(false, e, cond, mkExprS(e, res, newObj), Some(mkExprS(e, res, obj)))), res)

    case AssignOpApp(info, lhs, Op(_, text), right:FunExpr)
         if text.equals("=") && lhs.isName =>
  ...
  def funapp2ir(e: FunApp, env: Env, res: IRId, target: String): IRStmt = e match {
    case FunApp(info, v@VarRef(_, fid), args) =>
      ...
      new IRStmtUnit(NF.makeDummyAST("forConcolic"), List(toObject(v, obj, fir))++
        ...
        List(IF.makeArgs(e, arg, newargs.map(p => Some(p))),
          getBase(v, fun, fir),
          IF.makeCall(true, e, res, obj, fun, arg)))
          
    case FunApp(info, dot@Dot(i, obj, member), args) =>
      funapp2ir(setUID(FunApp(info, setUID(Bracket(i, obj, NF.makeStringLiteral(getSpan(member), member.text, "\"")),
                dot.getUID), args), e.getUID), env, res, target)
